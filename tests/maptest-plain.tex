...
\ifx\CFGLOADED\undefined
  ...
  \def\MAPSETJP{ipaex}%   jaEmbed
  \def\MAPSETJPVAR{}%     jaVariant
...
%% japanese
\ifx\MAPSETJP\undefined\else
\ifx\ucs\undefined
  \ADDSPECIAL{pdf:mapfile ptex-\MAPSETJP\MAPSETJPVAR.map}
  \if y\USEOTF
    \ADDSPECIAL{pdf:mapfile otf-\MAPSETJP.map}
  \fi
\else
  \ADDSPECIAL{pdf:mapfile uptex-\MAPSETJP\MAPSETJPVAR.map}
  \if y\USEOTF
    \ADDSPECIAL{pdf:mapfile otf-up-\MAPSETJP.map}
  \fi
\fi
\long\def\TESTJP{%
  日本語Japanese\par
