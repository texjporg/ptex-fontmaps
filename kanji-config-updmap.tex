...
\def\xx@parselang#1#2#3\@nil#4{%
  ...
  \edef\xx@tempc{#4\ifx\xx@tempb\xx@AI\space(AI0)\fi}%
...
また，このリストで\command{(AI0)}と付いているものは，Adobe-Identity0の
...
引数に\command{auto}を指定すると，上記のリストの中から
\emph{最初に見つかったフォント}を自動的に選択します。
例えば
\begin{verbatim}
    $ sudo kanji-config-updmap-sys auto
\end{verbatim}
では日本語フォントが\command{morisawa-pr6n}→\command{morisawa}→…の順に
探され，最初に見つかったフォントを埋め込むように自動で設定します。
\begin{verbatim}
